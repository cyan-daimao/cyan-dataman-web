...
import {
  ...
  SubjectDTO,
  treeSubjects,
} from "../../../api/MetadataSubjectAPI.ts";
...
const IcebergTableForm: React.FC<IcebergTableFormProps> = ({
  ...
  const current: EmployeeDTO = JSON.parse(
    String(localStorage.getItem("current")),
  );
  ...
  const fetchSubjects = async () => {
    try {
      const subjects = await treeSubjects();
        ...
        >
          <Input
            ...
            prefix={
              ...
              >
                {Form.useWatch("layerCode", form)}_
                {Form.useWatch("subjectCode", form)}_
              </span>
            }
          />
        </Form.Item>
          ...
          <Col span={16}>
            ...
            >
              <Cascader
                ...
                changeOnSelect
                onChange={(value) => {
                  // Cascader 返回的是路径数组，取最后一个值作为单选结果
                  const selectedValue = Array.isArray(value)
                    ? value[value.length - 1]
                    : value;
                  console.log("选中节点值:", selectedValue);
                  form.setFieldsValue({ subjectCode: selectedValue });
                }}
